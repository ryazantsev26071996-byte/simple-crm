    ...
    try {
      ...
      if (valid.length > 0) {
        await apiFetch(supabase, "recurring_task_checklist", {
          method: "POST",
          body: JSON.stringify(valid.map((it, i) => ({ task_id: tid, item: it.text.trim(), sort_order: i }))),
        });
      }
...
export function RecurringTasksAdmin({ employees, supabase }) {
  ...
  async function handleEdit(task) {
    try {
      const checklist = await apiFetch(supabase, `recurring_task_checklist?task_id=eq.${task.id}&order=sort_order.asc&select=id,item,sort_order`);
  ...
  async function load() {
    ...
    try {
      ...
      const [taskRows, clRows, logRows] = await Promise.all([
        ...
        apiFetch(supabase, `recurring_task_checklist?task_id=in.(${taskIds.join(",")})&order=sort_order.asc&select=id,task_id,item`),
