...
function MailingsPopup({ client, onClose }) {
...
export default function ClientCard({ client, clients, role, authorName, userId, userEmail, onUpdate, onDelete, onClose, asModal = false }) {
  ...
  const [showMailings, setShowMailings] = React.useState(false);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: asModal ? '100%' : 'auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {role === 'admin' && (
            <button onClick={() => setShowMailings(true)}
      ...
      {showMailings && <MailingsPopup client={client} onClose={() => setShowMailings(false)} />}
    </div>
  );
